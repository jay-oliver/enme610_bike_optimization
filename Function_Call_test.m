...
energy_ex=energy_sum(trailsTheta.(fields(test_i)),trailsX.(fields(test_i)),v,GR,p,m)
...
for i=lb(1):0.1:ub(1)
    ...
    for j=lb(2):0.1:ub(2)
        ...
        for k=lb(3):0.1:ub(3)
            sz(3)=length(k);
            esum=esum+energy_sum(trailsTheta.(fields(test_i)),trailsX.(fields(test_i)),v,GR,p,m);
        end
    end
end
